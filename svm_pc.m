function svm_pc(sid, glm_folder, nFolds, th_ratio, radius, events, e1, e2, rndc)
% sid = 4-digit string
...
for sess = 1:5
    ...
    movefile('searchlight_0001.nii', ...
        sprintf('corr_f%i_%i_%s_b%i_%s%s_%imm_%s%s.nii', ...
        nFolds, th_ratio, sid, sess, events{e1}, events{e2}, radius, k_func, postfix));
    movefile('searchlight_0002.nii', ...
        sprintf('sens_f%i_%i_%s_b%i_%s%s_%imm_%s%s.nii', ...
        nFolds, th_ratio, sid, sess, events{e1}, events{e2}, radius, k_func, postfix));
    movefile('searchlight_0003.nii', ...
        sprintf('spec_f%i_%i_%s_b%i_%s%s_%imm_%s%s.nii', ...
        nFolds, th_ratio, sid, sess, events{e1}, events{e2}, radius, k_func, postfix));
