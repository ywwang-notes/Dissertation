...
for sess = 1:5 
    ...
    TRs{1} = [TRs{1}; TRs{4}]; % stay
    TRs{2} = [TRs{2}; TRs{3}]; % switch
    TRs(3:4) = [];
...
%           pt = strfind(SPM.xY.P(tr, :), sid);
    ...
    test_func = @(Y,XYZ, n, label, nFolds, k_func) sbj_cMVPA(Y, n, label, 1, 2, nFolds, k_func);
    searchopt = struct('def','sphere','spec', radius);
    spm_searchlight(Scans,searchopt, test_func, n, label, nFolds, k_func);
    toc
    
    movefile('searchlight_0001.nii', ...
        sprintf('corr_pc_%s_b%i_ss_%imm_%s.nii', ...
        sid, sess, radius, k_func));
    movefile('searchlight_0002.nii', ...
        sprintf('sens_pc_%s_b%i_ss_%imm_%s.nii', ...
        sid, sess, radius, k_func));
    movefile('searchlight_0003.nii', ...
        sprintf('spec_pc_%s_b%i_ss_%imm_%s.nii', ...
        sid, sess, radius, k_func));
