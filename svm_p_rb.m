function svm_p_rb(sid, glm_folder, radius, events, e1, e2, rndc)
% sid = 4-digit string
...
%            Scans.xY.VY(i,:) = [data_path SPM.xY.P(tr, pt:end-2)];
    ...
    movefile('searchlight_0001.nii', ...
        sprintf('corr_p_%s_b%i_%s%s_%imm_%s%s_rb.nii', ...
        sid, sess, events{e1}, events{e2}, radius, k_func, postfix));
    movefile('searchlight_0002.nii', ...
        sprintf('sens_p_%s_b%i_%s%s_%imm_%s%s_rb.nii', ...
        sid, sess, events{e1}, events{e2}, radius, k_func, postfix));
    movefile('searchlight_0003.nii', ...
        sprintf('spec_p_%s_b%i_%s%s_%imm_%s%s_rb.nii', ...
        sid, sess, events{e1}, events{e2}, radius, k_func, postfix));
