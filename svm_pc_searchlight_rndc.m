% "chunck-wise" random permutation 
clear all
...
for sess = 1:5
    ...
    for iTRs = 1:c
        ...
        for t = 1:length(TRs{iTRs})
            tr = TRs{iTRs}(t);
            if tr - previous > 1
                pick = randperm(c); %randomly switch to a label
            end
            previous = tr;
            rndTRs{pick(1)} = [rndTRs{pick(1)} tr];
        end
    end
...
%           pt = strfind(SPM.xY.P(tr, :), sid);
    ...
    movefile('searchlight_0001.nii', ...
        sprintf('corr_pc_%s_b%i_%s%s_%imm_%s_rndc.nii', ...
        sid, sess, events{e1}, events{e2}, radius, k_func));
    movefile('searchlight_0002.nii', ...
        sprintf('sens_pc_%s_b%i_%s%s_%imm_%s_rndc.nii', ...
        sid, sess, events{e1}, events{e2}, radius, k_func));
    movefile('searchlight_0003.nii', ...
        sprintf('spec_pc_%s_b%i_%s%s_%imm_%s_rndc.nii', ...
        sid, sess, events{e1}, events{e2}, radius, k_func));
end % sess